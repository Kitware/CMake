...
bool cmListFileParser::DoParse()
{
  ...
  while (cmListFileLexer_Token* token = Scan()) {
...
bool cmListFile::ParseString(const char* content, const char* filename,
                             cmMakefile* mf)
{
...
bool cmListFile::ParseFile(const char* filename, bool topLevel, cmMakefile* mf)
{
  ...
  if (topLevel) {
    ...
    if (!hasProject) {
      ...
      cmListFileArgument prj("Project", cmListFileArgument::Unquoted, 0, 0);
...
bool cmListFileParser::ParseFunction(cmListFileLexer_Token* firstToken)
{
  ...
  while ((token = Scan()) && token->type == cmListFileLexer_Token_Space) {
  }
  if (!token) {
    if (this->IsString) {
  ...
  while (
    (lastLine = cmListFileLexer_GetCurrentLine(this->Lexer), token = Scan())) {
    if (token->type == cmListFileLexer_Token_Space ||
...
bool cmListFileParser::AddArgument(cmListFileLexer_Token* token,
                                   cmListFileArgument::Delimiter delim)
{
  cmListFileArgument a(token->text, delim, token->line, token->column);
