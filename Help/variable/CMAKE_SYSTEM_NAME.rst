...
``WindowsKernelModeDriver`` Windows Kernel-Mode Driver
...
* When building drivers for Kernel-Mode Driver Framework on Windows, toolchain
  file can set ``CMAKE_SYSTEM_NAME`` value to ``WindowsKernelModeDriver``. See
  also the :variable:`CMAKE_WINDOWS_KMDF_VERSION` variable.
